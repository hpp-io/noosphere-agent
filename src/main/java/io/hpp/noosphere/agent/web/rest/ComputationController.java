...
public class ComputationController {
    ...
    //    @PostMapping(value = "/Computations/stream", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
    //    public CompletableFuture<ResponseEntity<StreamingResponseBody>> createComputationStream(
    ...
    //            Map<String, Object> messageData = new HashMap<>(requestData);
    //            messageData.put("id", ComputationId);
    //            messageData.put("ip", clientIp);
    //
    //            OffchainComputationMessage message = parseOffchainMessage(messageData);
    //            Object filtered = guardian.processMessage(message);
    //
    ...
    //            }
    //
    //            if (message.getType() != MessageType.OFFCHAIN_Computation) {
    //                return CompletableFuture.completedFuture(
    //                    ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).build());
    //            }
    //
    //            log.debug("Processed REST response - endpoint: /api/Computations/stream, " +
    //                    "method: POST, status: 200, type: {}, id: {}",
    //                message.getType(), message.getId());
    //
    //            StreamingResponseBody streamingBody = outputStream -> {
    //                try {
    //                    // First write Computation ID
    //                    outputStream.write((message.getId() + "\n").getBytes());
    //                    outputStream.flush();
    //
    //                    // Then stream Computation results
    //                    orchestratorService.processStreamingComputation(message)
    //                        .doOnNext(chunk -> {
    //                            try {
    //                                outputStream.write(chunk);
    //                                outputStream.flush();
    //                            } catch (Exception e) {
    //                                log.error("Error writing streaming data", e);
    //                            }
    //                        })
    ...
    //
    //            return CompletableFuture.completedFuture(
    //                ResponseEntity.ok(streamingBody));
    //
    //        } catch (Exception e) {
    //            log.error("Error creating streaming Computation", e);
    //            return CompletableFuture.completedFuture(
